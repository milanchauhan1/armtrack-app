  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center px-5">
      <motion.div
        ...
        style={{ maxWidth: 400 }}
      >
        <div className="rounded-2xl p-7 flex flex-col gap-6" style={{ backgroundColor: "#111111", border: "1px solid #222222", boxShadow: "0 0 40px rgba(59,130,246,0.08)" }}>
          ...
          {!loggedIn && !invalidCode && (
            <div className="flex flex-col gap-5">
              <div>
                ...
                <h1 className="text-xl font-extrabold tracking-tight text-white mb-1">You&apos;re invited!</h1>
                <p className="text-sm" style={{ color: "#888888" }}>You need an account to join this team.</p>
              </div>
              <div className="flex items-start gap-3 rounded-xl px-4 py-3" style={{ backgroundColor: "rgba(59,130,246,0.08)", border: "1px solid rgba(59,130,246,0.2)" }}>
                <AlertCircle size={15} style={{ color: "#60a5fa", flexShrink: 0, marginTop: 1 }} />
                <p className="text-sm" style={{ color: "#888888" }}>
                  Your invite code <span className="font-bold text-white">{rawCode.toUpperCase()}</span> will be saved automatically.
                </p>
              </div>
              <Link href="/signup" className="w-full rounded-2xl py-3.5 text-sm font-bold text-white text-center transition-all hover:opacity-90" style={{ backgroundColor: "#3B82F6", boxShadow: "0 4px 28px rgba(59,130,246,0.35)" }}>
                Sign Up
              </Link>
          ...
          {loggedIn && alreadyOnTeam && (
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-sm" style={{ color: "#888888" }}>You&apos;re already on a team. Visit your dashboard.</p>
              </div>
              <Link href="/dashboard" className="w-full rounded-2xl py-3.5 text-sm font-bold text-white text-center transition-all hover:opacity-90" style={{ backgroundColor: "#3B82F6", boxShadow: "0 4px 28px rgba(59,130,246,0.35)" }}>
                Go to Dashboard
              </Link>
            </div>
          )}
          ...
          {invalidCode && (
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-sm" style={{ color: "#888888" }}>This invite link is no longer valid. Ask your coach for a new one.</p>
              </div>
              <Link href="/dashboard" className="w-full rounded-2xl py-3.5 text-sm font-bold text-center transition-all" style={{ backgroundColor: "#111111", border: "1px solid #222222", color: "#888888" }}>
                Go to Dashboard
              </Link>
            </div>
          )}
          ...
          {loggedIn && !alreadyOnTeam && !invalidCode && teamInfo && (
            <div className="flex flex-col gap-5">
              {joined ? (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center gap-3 py-4 text-center">
                ...
                <>
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-400 mb-1">Team Invite</p>
                    <h1 className="text-xl font-extrabold tracking-tight text-white mb-1">{teamInfo.name}</h1>
                    <p className="text-sm" style={{ color: "#888888" }}>Coach: {teamInfo.coach_name}</p>
                  </div>
